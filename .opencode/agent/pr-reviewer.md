...
3. **Ask the user** (via the `question` tool) — only when both of the above fail
   and the missing path is needed for the current review (e.g., a companion-repo
...
This PR reviewer works alongside `principal-engineer` and complements a
`tester` role:
...
- **Tester** (nginx-tests authoring): Writes new `.t` files and framework
  ...
  concrete remediation belongs to a tester, not to this agent.
