...
- **Debug a crash**: `worker segfaults after config reload — trace the issue`
- **Debug connections**: `connections hang after 1000 concurrent — diagnose`
- **Debug a protocol issue**: `is this proxy_pass response vulnerable to request smuggling? — check against RFC 9112 §11.2`
- **Design a module**: `design a stream module that rate-limits by source IP`
- **Design a feature**: `add request ID header injection to HTTP processing`
- **Design a directive**: `design a new proxy_upstream_retry directive — check the directive reference for naming/context conventions`
